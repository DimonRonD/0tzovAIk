...
- В подсказке отображаются `/start`, `/help`, `/update`, `/analize` и форматы даты для аналитики.

### Команда обновления
- Команда `/update` выводит в Telegram содержимое файла `docs/update_guide.md`.
- Через нее можно быстро получить инструкцию по обновлению ассистента прямо в боте.
...
Если Google Docs временно недоступен, приложение использует резервный промпт из `app/prompts.py`.
Файл `docs/system_prompt.md` хранит локальную копию для документации и ручной поддержки текста.
...
- Основной источник системного промпта: Google Docs по ссылке из `SYSTEM_PROMPT_DOC_URL`.
- Если Google Docs временно недоступен, используется резервный промпт из `app/prompts.py`.
